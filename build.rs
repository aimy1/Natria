...
const PROMPT_MASK: &[u8] = b"NatriaPromptMask";

fn main() {
    println!("cargo:rerun-if-changed=src/prompts/natria.md");
    println!("cargo:rerun-if-changed=src/prompts/natria.hint.md");
    println!("cargo:rerun-if-changed=src/prompts/natria-dialogs.md");
    println!("cargo:rerun-if-changed=assets/o200k_base.tiktoken");
    println!("cargo:rerun-if-changed=assets/jieba/dict.txt");
    // Rerun on any source or frontend change so NATRIA_BUILD_ID uniquely
    ...
    let prompt = obfuscate("src/prompts/natria.md");
    let hint = obfuscate("src/prompts/natria.hint.md");
    let dialogs = obfuscate("src/prompts/natria-dialogs.md");
    let out_dir = env::var("OUT_DIR").expect("OUT_DIR is set by cargo");
    let dest = Path::new(&out_dir).join("default_natria_prompt.rs");
    fs::write(
        dest,
        format!(
            "const PROMPT_MASK: &[u8] = b\"NatriaPromptMask\";\n\
             const OBFUSCATED_DEFAULT_SYSTEM_PROMPT: &str = \"{prompt}\";\n\
             const OBFUSCATED_DEFAULT_NATRIA_HINT: &str = \"{hint}\";\n\
             const OBFUSCATED_DEFAULT_NATRIA_DIALOGS: &str = \"{dialogs}\";\n"
        ),
    )
