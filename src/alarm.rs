...
pub fn alarms_file(paths: &NatriaPaths) -> PathBuf {
    paths.state_dir.join("alarms.json")
}

pub fn alarm_log_file(paths: &NatriaPaths) -> PathBuf {
...
pub fn load(paths: &NatriaPaths) -> Result<Vec<AlarmRecord>> {
...
pub fn save(paths: &NatriaPaths, records: &[AlarmRecord]) -> Result<()> {
...
pub fn upsert(paths: &NatriaPaths, record: AlarmRecord) -> Result<()> {
...
pub fn update_status(paths: &NatriaPaths, id: &str, status: AlarmStatus) -> Result<()> {
    let mut records = load(paths)?;
...
pub fn remove(paths: &NatriaPaths, id: &str) -> Result<Option<AlarmRecord>> {
    let mut records = load(paths)?;
...
pub fn cleanup_dead(paths: &NatriaPaths) -> Result<Vec<AlarmRecord>> {
