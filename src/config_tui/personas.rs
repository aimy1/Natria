...
/// 默认 Natria 人格:本体只读,回车只编辑附属的防失忆提示与预设对话
/// (scope 固定为 default)。
pub(in crate::config_tui) fn edit_natria_persona_extras(
    stdout: &mut io::Stdout,
    paths: &NatriaPaths,
    config: &AppConfig,
) -> Result<()> {
    let (hint, dialogs) = crate::persona_hint::natria_aux_prefill(config, paths);
    let mut fields = persona_aux_fields(hint, dialogs, true);
    if !run_form(stdout, t(" NATRIA EXTRAS ", " Natria 人格附加 "), &mut fields)? {
        return Ok(());
    }
...
pub(in crate::config_tui) fn edit_miyu_persona_extras(
    ...
    edit_natria_persona_extras(stdout, paths, config)
}
